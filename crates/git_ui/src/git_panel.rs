        ...
        self.remote_operation_task = Some(
            cx.spawn_in(window, async move |this, cx| {
                ...
                if let Err(error) = operation(this.clone(), cx).await {
                    this.update(cx, |this, cx| {
                        this.show_error_toast("remote operation", error, cx);
                    })
                    .log_err();
                }
            })
...
mod tests {
    ...
    async fn test_remote_operation_serialization(cx: &mut TestAppContext) {
        ...
        assert!(matches!(
            panel.read(cx).pending_remote_operation,
        ...
        assert!(panel.read(cx).pending_remote_operation.is_none());
    }
